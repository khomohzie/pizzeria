...
async function signUp () {
    ...
    if (email.length < 1 || username.length < 1 || password1.length < 1 || password2.length < 1) {
    ...
    let request = await fetch('https://pizzeria-oop.herokuapp.com/api/user/auth/signup', {
        method : 'post',
        headers : {
    ...
    if (!request || !request.success) { return alert('Failed to create user') }
    ...
    let login = await fetch('https://pizzeria-oop.herokuapp.com/api/auth/signin', {
        method : 'post',
        headers : {
            'Content-Type' : 'application/json'
        },
    ...
    if (!login || !login.success) {return alert('Login failed')}
...
async function signIn () {
    ...
    if (email.length < 1 || password.length < 1) {return alert('A valid e-mail and password must be entered')}
